    ...
    document.automaton.kind === 'pda'
      ...
      : document.automaton.kind === 'turing'
        ? {
            ...
            write: '',
            move: 'R' as const,
            ...(document.automaton.tapes > 1
              ? {
                  tapeActions: Array.from({ length: document.automaton.tapes }, () => ({
                    read: '',
                    write: '',
                    move: 'R' as const,
                  })),
        ...
        <table v-else>
          ...
          <tbody>
            <tr v-for="state in document.automaton.states" :key="state.id">
              ...
              <td>
                {{
                  [state.isInitial ? 'initial' : '', state.isFinal ? 'final' : '']
                    .filter(Boolean)
                    .join(', ') || 'regular'
                }}
              </td>
              ...
              <td>
                <button type="button" @click="document.select({ type: 'state', id: state.id })">
                  Select {{ state.name }}
                </button>
              </td>
            </tr>
      ...
      <section aria-labelledby="transitions-heading">
        ...
        <form class="diagram-outline__add-transition" @submit.prevent="addTransition">
          <label
            ...
            <select v-model="fromId">
              ...
              <option v-for="state in document.automaton.states" :key="state.id" :value="state.id">
                {{ state.name }}
              </option>
            </select></label
          >
        ...
        <ul class="diagram-outline__transitions">
          <li v-for="transition in document.automaton.transitions" :key="transition.id">
            <button
              type="button"
              @click="document.select({ type: 'transition', id: transition.id })"
            >
              {{ document.automaton.states.find((state) => state.id === transition.from)?.name }} to
              {{ document.automaton.states.find((state) => state.id === transition.to)?.name }},
              read {{ transition.read || 'ε' }}
            </button>
          </li>
        </ul>
      </section>
...
.diagram-outline > summary {
  padding: 7px 12px;
  cursor: pointer;
  font-weight: 600;
}
.diagram-outline__content {
  display: grid;
...
.diagram-outline button,
.diagram-outline select {
  min-height: 30px;
...
.diagram-outline button:focus-visible,
.diagram-outline select:focus-visible,
.diagram-outline > summary:focus-visible {
  outline: 2px solid var(--color-primary);
  outline-offset: 2px;
}
.diagram-outline__add-transition {
  display: flex;
  flex-wrap: wrap;
  align-items: end;
  gap: 8px;
}
.diagram-outline__add-transition label {
  display: grid;
  gap: 3px;
}
.diagram-outline__transitions {
  display: flex;
  flex-wrap: wrap;
  gap: 5px;
  padding: 6px 0;
